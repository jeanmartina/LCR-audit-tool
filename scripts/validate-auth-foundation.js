...
const rateLimit = read('src/auth/rate-limit.ts');
const requestSecurity = read('src/auth/request-security.ts');
...
if (mode === 'schema') {
  ...
  assertIncludes(runtimeStore, 'create table if not exists rate_limit_events', 'Missing persisted rate limit schema');
...
if (mode === 'auth') {
  ...
  assertIncludes(authSession, 'serializeSessionCookie', 'Missing centralized session cookie helper');
  assertIncludes(authSession, 'SESSION_COOKIE_SECURE', 'Missing session cookie secure override');
  assertIncludes(authSession, '; Secure', 'Missing secure session cookie support');
  assertIncludes(rateLimit, 'recordRateLimitAttempt', 'Missing database-backed rate limit helper usage');
  assertIncludes(runtimeStore, 'recordRateLimitAttempt', 'Missing persisted rate limit implementation');
  assertIncludes(requestSecurity, 'rejectCrossOriginRequest', 'Missing same-origin mutation guard helper');
  assertIncludes(requestSecurity, 'sec-fetch-site', 'Missing Fetch Metadata same-origin guard');
  assertIncludes(groupsRoute, 'rejectCrossOriginRequest(request)', 'Missing same-origin guard on auth group mutation route');
  assertIncludes(platformRoute, 'rejectCrossOriginRequest(request)', 'Missing same-origin guard on platform settings mutation route');
  assertIncludes(providerStatusRoute, 'rejectCrossOriginRequest(request)', 'Missing same-origin guard on provider status mutation route');
